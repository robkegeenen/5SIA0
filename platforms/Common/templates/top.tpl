...
(	
	...
	`ifdef INCLUDE_STATE_CONTROL	//use only if state control is enabled		
		...
		#(
			.DATA_WIDTH(D_WIDTH),
			.ADDR_WIDTH(<<STATE_ADDR_RANGE>>),
			.DATAFILE(""),
			.ADDRESSABLE_SIZE(<<STATE_ADDR_RANGE>>),
			.DO_INIT(0)
		)	
		 STATE_MEM_inst
		(
			.clock(iClk),
			.data(wSM_WriteData),
			.rdaddress(wSM_Address[<<STATE_ADDR_RANGE>>-1+INTERFACE_BYTE_ENABLES_WIDTH:INTERFACE_BYTE_ENABLES_WIDTH]),
			.wraddress(wSM_Address[<<STATE_ADDR_RANGE>>-1+INTERFACE_BYTE_ENABLES_WIDTH:INTERFACE_BYTE_ENABLES_WIDTH]),
