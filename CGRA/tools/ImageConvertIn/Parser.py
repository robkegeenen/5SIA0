...
class Parser():
	...
	self.instructionRegexes['function_label']=[('target',self.function_label_token, "")]
    ...
    def parseParallel(self, lines):
        ...
        for lineNumber, line in enumerate(lines):
            ...
            if active_section=='.text':
                ...
                labelInCol=None
                for idx, scol in enumerate(scolumns):
                    if re.match("^\s*"+self.label_token+"\s*", scol) or re.match("^\s*"+self.function_label_token+"\s*",scol):
                        labelInCol=idx
                        break
