...
static const int32_t inva[order+1] = {-65536, 0, 64572, 0, -63818, 0, 25323, 0, -7287, 0, 740};
  ...
  while(np--)
    {
      ...
      for (j=0;j<order+1;j++)
        yy = yy + (int32_t)(((int64_t)(b[j])*(int64_t)(x[i-j])) >> SCALE);
      for (j=1;j<order+1;j++) {
        int32_t y1 = -(int32_t)(((int64_t)(a[j])*(int64_t)(y[i-j])) >> SCALE);
        int32_t y2 =  (int32_t)(((int64_t)(inva[j])*(int64_t)(y[i-j])) >> SCALE);
        printf("%d - %d\n", y1, y2);
        //yy = yy - y1;
      }
      *yo++ = y[i] = yy;
